---
category: Projects
date: '2023-11-01'
hierarchy: Projects/CubeSat
layout: default
permalink: /Projects/CubeSat/
tags:
- SolidWorks
- Ansys
- Aerospace
title: CubeSat Structure
---

## CubeSat Structure

Structural and component design for a research satellite prototype. Validated for launch loads and orbital thermal conditions.

## Structure & Analysis

Designed the primary structure using <i>SolidWorks</i>. Conducted structural analysis in <i>Ansys</i> to ensure payload integrity under launch vibration and acceleration loads. Contributed to tooling for <u>active space debris removal</u>.

## Thermal Validation

Simulated LEO thermal cycling (-150°C to +120°C) to verify component survivability. Executed FEA validation across 50+ load cases.

## Integration

Precision assembly and contamination control. Integration sequence: [https://placehold.co/350/2563eb/FFF?text=Assembly+1, https://placehold.co/350/7c3aed/FFF?text=Assembly+2, https://placehold.co/350/db2777/FFF?text=Assembly+3] Mission patch designed with York Space Engineering.
