...
function updateHeaderState() {
    ...
    // 1. Scroll Logic: Hide Brand row if scrolled down > 50px
    if (scrollY > 50) {
...
function handleSearch(q) { 
    if(!q) return; 
    isSearchActive = true; 
    // Reset secondary/tertiary navs visually on search
    document.getElementById('sub-nav').classList.add('collapsed');
...
function buildTertiaryNav(top, secondary) {
    const n = document.getElementById('tertiary-nav'); if(!n) return; n.innerHTML = '';
    const prefix = `${top}/${secondary}`;
    
    const tertPages = [...new Set(db.filter(r => r.Page && r.Page.startsWith(prefix + '/')).map(r => r.Page.split('/').slice(0, 3).join('/')))].sort();
...
function handleRouting() { 
    ...
    const parts = h.split('/');
    const top = parts[0]; 
    const sec = parts.length > 1 ? parts[1] : null;
    ...
    // Sub/Tert Logic
    if(top && top !== 'Home' && !h.startsWith('Filter:')) {
        buildSubNav(top);
        if(sec) buildTertiaryNav(top, sec);
        else document.getElementById('tertiary-nav').classList.remove('active');
    } else {
        document.getElementById('sub-nav').classList.add('collapsed');
        document.getElementById('tertiary-nav').classList.remove('active');
    }
    
    // Recalc header height for padding
...
function renderIndex() {
    ...
    for(const [cat, items] of Object.entries(groups)) {
        ...
        // Sort items alphabetically within group
        items.sort();
        ...
        items.forEach(p => {
            ...
            const depth = parts.length; // 1 = Main, 2 = Sub, 3 = Tert
            
            const indentClass = depth >= 3 ? 'indent-level-2' : '';
...
function renderTimeline() {
    ...
    const validRows = db.filter(r => r.Timestamp && r.Page !== 'Home' && r.Page !== 'Footer');
    validRows.sort((a, b) => new Date(b.Timestamp) - new Date(a.Timestamp));

    // Group by Month Year
    const groups = {};
    validRows.forEach(r => {
        const d = formatDate(r.Timestamp); // "JAN 2025"
    ...
    Object.keys(groups).forEach(dateStr => {
        ...
        contentCol.addEventListener('scroll', () => {
            if (contentCol.scrollLeft + contentCol.clientWidth >= contentCol.scrollWidth - 5) {
                contentCol.classList.add('end-reached');
            } else {
                contentCol.classList.remove('end-reached');
            }
        });

        // Render Cards inside
        // Reuse logic from renderRows but manually appending
        items.forEach(r => {
            const card = createGridCard(r);
        ...
        // Trigger check for fade immediately
        if (contentCol.scrollWidth <= contentCol.clientWidth) {
             contentCol.classList.add('end-reached');
        }
    });
...
function renderHome() { 
    ...
    let allRecents = db.filter(r => r.Page !== 'Home' && r.Page !== 'Footer')
                       .sort((a, b) => new Date(b.Timestamp || 0) - new Date(a.Timestamp || 0));

    // FEATURED PINS LOGIC
    const featured = allRecents.filter(r => r.Tags && r.Tags.toLowerCase().includes('featured'));
    const others = allRecents.filter(r => !r.Tags || !r.Tags.toLowerCase().includes('featured'));
    
    // Take top 6 total, starting with featured
    const finalRecents = [...featured, ...others].slice(0, 6);

    if(finalRecents.length > 0) { renderRows(finalRecents, "Recent Activity", true); } 
}

// Helper to isolate card creation logic for reuse in Timeline
function createGridCard(r) {
    ...
    let catClass = '';
    const pLower = r.Page ? r.Page.toLowerCase() : '';
...
function renderRows(rows, title, append, forceGrid, isArticleMode = false) {
    ...
    rows.forEach(r => {
        ...
        // Reuse createGridCard logic for standard grid items? 
        // Partially, but renderRows handles mixed types (hero, text, quote).
        // For standard card logic, we can defer to the createGridCard logic if it matches.
        ...
        if(!forceGrid) {
            ...
            if(isArticleMode && (!r.SectionType || r.SectionType === 'card')) {
                 // Article Mode Logic
                 const d = document.createElement('div'); d.className = 'section layout-text';
                 let mediaHtml = '';
                 const modelMatch = r.Content ? r.Content.match(/\{\{(?:3D|STL): (.*?)(?: \| (.*?))?\}\}/i) : null;
                 ...
                 let contentHtml = processText(r.Content);
                 if(modelMatch) contentHtml = contentHtml.replace(/<div class="embed-wrapper stl".*?<\/div>/, '');
        ...
        // Fallback to Standard Grid Card
        if(gc) gc.appendChild(createGridCard(r));
    });
